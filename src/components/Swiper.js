...
//     <><div className="mx-auto w-screen ">
      ...
      //   className="mySwiper h-[500px]  max-md:h-[245px]"
      //   style={{
...
const ImageSlider = ({ folder }) => {
  const [images, setImages] = useState([]);

  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get(`/api/images?folder=sliderimages`);
  ...
  return (
    <div>
      <Swiper
        ...
        navigation={true}
        modules={[Autoplay, Pagination, Navigation]}
        className="mySwiper h-[600px]   max-md:h-[245px]"
        style={{
          '--swiper-navigation-color': '#27AB61', // Replace with your color
        }}
      >
        {images.map((image) => (
          <SwiperSlide key={image.public_id} className="h-[450px] w-screen max-md:h-[240px] ">
            <img className='h-[550px] w-full max-md:h-[240px] ' src={image.secure_url} alt={image.public_id} style={{ width: '100%' }} />
          </SwiperSlide>
        ))}
      </Swiper>
    </div>
